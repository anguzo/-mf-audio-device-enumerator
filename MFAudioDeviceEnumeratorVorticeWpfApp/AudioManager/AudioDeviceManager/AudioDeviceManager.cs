...
using MFAudioDeviceEnumeratorVorticeWpfApp.AudioManager.AudioDeviceManager.Interop;
...
{
    public class AudioDeviceManager : CallbackBase, IAudioDeviceManager
    {
        ...
        public AudioDeviceManager(AudioDeviceKind kind)
        {
            ...
            _enumerator = new IMMDeviceEnumerator();
            _enumerator.RegisterEndpointNotificationCallback(this);
            var devices = _enumerator.EnumAudioEndpoints(Flow);
        ...
        void IMMNotificationClient.OnDeviceStateChanged(string pwstrDeviceId, int newState)
        {
            switch ((DeviceStates)newState)
            {
                case DeviceStates.Active:
                    ((IMMNotificationClient)this).OnDeviceAdded(pwstrDeviceId);
                    break;
                case DeviceStates.Disabled:
                case DeviceStates.NotPresent:
                case DeviceStates.Unplugged:
                    ((IMMNotificationClient)this).OnDeviceRemoved(pwstrDeviceId);
                    break;
                default:
                    Debug.WriteLine($"Unknown DEVICE_STATE: {newState}");
                    break;
            }
        }
